 ...
 *
 * Lossless/high-bitrate sources are re-encoded to AAC-LC ~256kbps: watches decode AAC in hardware
 * but most FLAC decoding on Wear OS SoCs is software-only, and lossless files are also far larger
 * to transfer and store on a watch's very limited flash. Sources that are already a compressed
 * lossy format at or below the target bitrate are sent through untouched (see
