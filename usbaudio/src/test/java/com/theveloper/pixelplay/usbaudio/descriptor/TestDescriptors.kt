...
/**
 * UAC2 DAC with an XMOS-style dual-clock tree, modeled on FiiO/Topping designs:
 * clock selector 0x28 routes pin 1 → clock source 0x29 (44.1 kHz family) and
 * pin 2 → clock source 0x2A (48 kHz family); the streaming terminal references the
 * SELECTOR (not a source) via bCSourceID. One 32-bit async alt setting with feedback.
 */
internal fun uac2SelectorDacDescriptors(): ByteArray {
    val blob = DescriptorBlob()
    deviceAndConfigHeader(blob)

    // Interface 0 alt 0: AudioControl (UAC2)
    blob.descriptor(0x04, 0x00, 0x00, 0x00, 0x01, 0x01, 0x20, 0x00)
    // AC HEADER: bcdADC 2.00
    blob.descriptor(0x24, 0x01, 0x00, 0x02, 0x0A, 0x60, 0x00, 0x00)
    // CLOCK_SOURCE 0x29 (44.1k family) + 0x2A (48k family), freq host-programmable
    blob.descriptor(0x24, 0x0A, 0x29, 0x03, 0x07, 0x00, 0x00)
    blob.descriptor(0x24, 0x0A, 0x2A, 0x03, 0x07, 0x00, 0x00)
    // CLOCK_SELECTOR 0x28: 2 pins → 0x29, 0x2A; selector control host-programmable
    blob.descriptor(0x24, 0x0B, 0x28, 0x02, 0x29, 0x2A, 0x03, 0x00)
    // INPUT_TERMINAL 0x01: USB streaming, clock = SELECTOR 0x28, 2 channels
    blob.descriptor(
        0x24, 0x02, 0x01, lo(0x0101), hi(0x0101), 0x00, 0x28, 0x02,
        0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
    )
    // FEATURE_UNIT 0x0B: master mute+volume programmable
    blob.descriptor(
        0x24, 0x06, 0x0B, 0x01,
        0x0F, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
    ...
    // OUTPUT_TERMINAL 0x03: headphones, source 0x0B, clock 0x28
    blob.descriptor(0x24, 0x03, 0x03, lo(0x0302), hi(0x0302), 0x00, 0x0B, 0x28, 0x00, 0x00, 0x00)

    // Interface 1 alt 0: AudioStreaming zero-bandwidth
    blob.descriptor(0x04, 0x01, 0x00, 0x00, 0x01, 0x02, 0x20, 0x00)
    // Interface 1 alt 1: 32-bit subslot
    blob.descriptor(0x04, 0x01, 0x01, 0x02, 0x01, 0x02, 0x20, 0x00)
    blob.descriptor(
        0x24, 0x01, 0x01, 0x05, 0x01, 0x01, 0x00, 0x00, 0x00,
        0x02, 0x03, 0x00, 0x00, 0x00, 0x00
